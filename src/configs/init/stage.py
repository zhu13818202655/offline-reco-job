...
def stage_config() -> AllInOneConfig:
    return AllInOneConfig(
        base_dag=BaseDagConfig(dag_id='base', dag_name='基础DAG', base=_stage_base_config()),
        reco_dags=[
            RecoDagConfig(
                dag_id='reco_credit_card_ca_0000',
                dag_name='信用卡获客DAG 0000',
                scene=_stage_credit_app_scene(),
            ),
            RecoDagConfig(
                dag_id='reco_debit_card_ca_0000',
                dag_name='借记卡获客DAG 0000',
                scene=_stage_debit_app_scene(),
            ),
            RecoDagConfig(
                dag_id='reco_debit_card_ca_0001',
                dag_name='借记卡获客DAG 0001(短信渠道)',
                scene=_stage_debit_sms_scene(),
            ),
            RecoDagConfig(
                dag_id='reco_credit_cancel_0000', dag_name='信用卡销户场景', scene=_stage_cancel_scene(),
            ),
            RecoDagConfig(dag_id='reco_adhoc_0000', dag_name='资金流失场景', scene=adhoc_scene(),),
        ],
        post_proc_dags=[_stage_sms_post_proc(), _stage_app_post_proc(),],
    )


_STAGE_MODEL_DIR = f'/user/tech/{Env.value}/model'


def _stage_base_config() -> BaseConfig:
    base = BaseConfig(
        react_delay=2,
        data_keep_day=30,
        ...
        feat_engines=_stage_feat_engines(),
        ...
        test_banners=[
            ...
            '71_bbcreditCard_tjbk',
            '71_zxcreditCard_tjbk',
        ],
    )
    return base
...
def _stage_feat_engines() -> List[FeatureEngineConfig]:
    return [
        FeatureEngineConfig(
...
def _stage_credit_app_scene() -> SceneConfig:
    return _stage_scene_cfg(
...
def _stage_debit_app_scene() -> SceneConfig:
    return _stage_scene_cfg(
        id=S_DEBIT.scene_id,
...
def _stage_debit_sms_scene() -> SceneConfig:
    return _stage_scene_cfg(
        id=S_DEBIT.scene_id,
        name=S_DEBIT.scene_name,
        user_filter={},
...
def _stage_cancel_scene() -> SceneConfig:
    return SceneConfig(
...
def _stage_sms_post_proc() -> PostProcessDagConfig:
...
def _stage_app_post_proc() -> PostProcessDagConfig:
    return PostProcessDagConfig(
